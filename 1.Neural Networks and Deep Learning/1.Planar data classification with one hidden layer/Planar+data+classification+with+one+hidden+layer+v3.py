...
def compute_cost(A2, Y, parameters):
    ...
    logprobs = np.multiply(np.log(A2),Y)
    cost = -1/m*(np.sum(logprobs) + np.sum(np.multiply(np.log(1-A2),(1-Y))))
    ### END CODE HERE ###
...
# the answer is not matching with the expected answer as there is defect in dataset on which team is working.
